...
@implementation PTInvocationHelper
...
+ (id _Nullable)performClass:(NSString * _Nonnull)className selector:(nonnull SEL)selector params:(NSDictionary *_Nullable)params {
    ...
    if (class != nil) {
        ...
        return [NSError errorWithDomain:TLInvocationDomain code:PTInvocationError_No_Class userInfo:nil];
    }
}
...
+ (id)invocationWithTarget:(id)target selector:(SEL)selector params:(NSDictionary *)params {
    ...
    if (![target respondsToSelector:selector] || (methodSig == nil)) {
        ...
        return [NSError errorWithDomain:TLInvocationDomain code:PTInvocationError_No_Selector userInfo:nil];
    }
    
    if (ACTION_PARAMS_COUNT(action) != (params == nil ? 0:1)) {
        NSLog(@"参数个数与方法不匹配");
        return [NSError errorWithDomain:TLInvocationDomain code:PTInvocationError_Param_Unmatch userInfo:nil];
    }
